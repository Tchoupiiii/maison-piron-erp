-- create_sale / create_sale_core : plafond de remise + contrôle explicite
--
-- create_sale_core ne bornait la remise que par « le total ne peut pas devenir
-- négatif » : rien n'empêchait une remise de 99% avec le droit ventes.remise.
-- Plafond choisi : 50% du total avant remise.
--
-- Par ailleurs, la permission ventes.remise n'était contrôlée qu'au niveau du
-- trigger private.guard_transaction_discount sur la table transactions — ça
-- protège déjà en pratique, mais le point d'entrée RPC public n'avait aucun
-- contrôle propre, uniquement l'erreur générique remontée par le trigger. On
-- ajoute le même contrôle dans create_sale, en défense en profondeur et pour
-- un message d'erreur cohérent avec les autres gardes du RPC.
...
begin
  ...
  if coalesce(discount_param, 0) > 0 and not private.has_permission('ventes.remise') then
...
begin
  ...
  -- Plafond de remise : 50% du total avant remise. Le trigger
  -- guard_transaction_discount contrôle déjà le DROIT d'accorder une remise ;
  -- ceci contrôle son AMPLEUR, qu'aucune des deux gardes existantes ne bornait.
